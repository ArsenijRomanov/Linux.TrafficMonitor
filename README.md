...
Небольшое **GUI‑приложение для Linux** на Python, которое в реальном времени перехватывает сетевые пакеты, определяет подозрительную активность по простым правилам и позволяет **блокировать/разблокировать источники** средствами **iptables**.
...
- **Агрегация статистики по источникам (src IP):**
  - суммарный объём трафика (байты/KB);
  - количество **уникальных портов назначения (dport)**.
- **Выявление подозрительных источников** по двум правилам:
  - превышение порога по суммарному объёму трафика (**Traffic Limit**);
  - превышение порога по числу уникальных портов (**Port Scanning**).
- **Блокировка/разблокировка IP** через iptables (цепочка **INPUT**).
- **Корректная остановка захвата пакетов** без перезапуска приложения (Scapy `AsyncSniffer`).
...
- **Linux** с доступными утилитами: `iptables`, `ip` (iproute2).
- **Python 3.10+**.
